...
import { capitalizeFirstLetter, formatChileanDateTime } from '../../../components/common/utils';
            ...
            <motion.div className='page-header' style={{maxHeight: '35vh'}} variants={itemVariants}>
                ...
                <div className='block-tile' style={{justifyContent: 'center'}}>
                {
                 complaint.complaintStatus === "RESOLVED" ? 
                  <ResolvedComplaintCard resolutionDetails={complaint.resolutionDetails}/>
                 :
                  (user.role === "ADMIN" ? (
                  <AdminComplaintOptions complaint={complaint}/>
                    ) : (
                  <DefaultComplaintOptions complaint={complaint} onClickBack={onClickBack}/>
                  ))
                }
                </div>
            </motion.div>
...
function ResolvedComplaintCard({ resolutionDetails }) {
  return (
    <div className='standout-list-tile-invert' style={{flexDirection: 'column', alignItems: 'start'}}>
        <div style={{fontWeight: 'bold', padding: '8px'}}> Resolution Details </div>
        <div className='divider-horizontal'/>
