...
export const ComplaintsProvider = ({ children }) => {
    ...
    const createComplaint = async (complaintData) => {
        try {
            const response = await createComplaintService(axiosInstance, complaintData);
    ...
    const updateComplaint = async (updatedComplaint, resolved = false) => {
        try {
            const apiUrl = resolved ? `/complaints/${updatedComplaint.complaintId}/resolve`
            ...
            setComplaints(prevComplaints => 
              prevComplaints.map(complaint => 
                complaint.id === updatedComplaintFromServer.id ? updatedComplaintFromServer : complaint
             )
            );
        } catch (error){
